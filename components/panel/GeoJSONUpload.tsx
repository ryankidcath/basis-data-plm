...
export default function GeoJSONUpload({ permohonanId, onSaved, embedded = false }: GeoJSONUploadProps) {
  ...
  async function processGeometries(geometries: { geom: GeoJSON.Geometry; nib?: string }[]) {
  ...
  async function handleFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      if (isDxf) {
        ...
        formData.set("layerName", "BIDANG");
        formData.set("permohonanId", permohonanId);
        const res = await fetch("/api/upload-dxf", { method: "POST", body: formData });
        const data = await res.json();
        if (!res.ok) {
          setError(data.error ?? "Gagal memproses DXF.");
          setUploading(false);
          if (fileInputRef.current) fileInputRef.current.value = "";
          return;
        }
        geometries = (data.geometries ?? []).map((p: { type: string; coordinates: number[][][] }) => ({
          geom: { type: "Polygon" as const, coordinates: p.coordinates },
        }));
      } else {
      ...
      await processGeometries(geometries);
      setSuccess(true);
    } catch (err) {
  ...
  return (
    <div className={wrapperClass}>
      <h3 className="text-sm font-medium text-navy-800 mb-2">Upload GeoJSON atau DXF Bidang Tanah</h3>
      <p className="text-xs text-navy-500 mb-3">
        Upload file GeoJSON (WGS84) atau DXF (layer BIDANG, koordinat TM-3 Zona 49.1 akan ditransformasi ke WGS84). Hanya closed LWPOLYLINE di layer tersebut yang dipakai.
      </p>
      {error && <p className="text-sm text-red-600 mb-2">{error}</p>}
      {success && <p className="text-sm text-green-600 mb-2">File berhasil disimpan. Peta akan diperbarui.</p>}
      <input
        ref={fileInputRef}
        type="file"
        accept=".geojson,.json,application/geo+json,application/json,.dxf"
        onChange={handleFile}
        disabled={uploading}
      ...
      />
      <p className="mt-2 text-xs text-navy-500">GeoJSON: WGS84. DXF: layer BIDANG, TM-3 Zona 49.1.</p>
    </div>
  );
}
