...
CEO brief (ceo-to-pm-005): User meminta **rekonstruksi layout dashboard GIS** (halaman setelah login) menjadi modern SaaS style dengan konsep **"Floating UI"** di atas peta. Login page sudah selesai (ceo-to-pm-004). Referensi: Linear/Vercel — desain ringan, luas, premium.
...
Ubah layout dashboard dari split-view (map kiri 52%, panel kanan 48%) menjadi **fullscreen map** dengan **floating card** di sisi kanan. Navbar ramping atau floating menu. Styling konsisten dengan estetika SaaS modern. Jangan ubah logika bisnis, autentikasi, atau flow data — hanya layout & styling.

## Constraints

- Next.js 14, TypeScript, Tailwind CSS
- Gunakan **Lucide React** untuk icon — jalankan `npm install lucide-react` di `project/basis-data-plm/` bila belum ada (saat ini tidak ada di package.json)
...
- `lucide-react` — install bila belum ada (`npm install lucide-react` di project/basis-data-plm)
- Leaflet, react-leaflet (existing)
- Tailwind (slate, backdrop-blur tersedia)

## Open Questions

- Floating menu vs navbar ramping — user memberi dua opsi; pilih yang lebih cocok untuk layout fullscreen map (Opsi A preferensi).
