...
CEO brief (ceo-to-pm-004): User meminta **rekonstruksi total halaman login** Basis Data PLM agar mencapai standar UI/UX SaaS modern (referensi: Linear, Vercel). Implementasi pernah dilakukan sebelumnya; pastikan hasil akhir sesuai spesifikasi di bawah. Tampilan harus bersih, profesional, hierarki visual kuat, terlihat "mahal" dan fungsional.

## Objective

Rekonstruksi `app/login/page.tsx` dengan desain modern SaaS sesuai spesifikasi detail di bawah. Hanya UI/UX — jangan ubah flow autentikasi Supabase.
...
- Halaman login existing: `project/basis-data-plm/app/login/page.tsx` (atau path yang sesuai di workspace)
- Supabase auth flow (tetap)
- Tailwind config (slate, indigo tersedia di default Tailwind)

## Open Questions

- Link "Lupa kata sandi?" — user tidak spesifik tujuan. Bisa pakai `/forgot-password` atau placeholder; halaman forgot-password scope terpisah.
